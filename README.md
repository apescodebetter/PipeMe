...
**1.8** — Document purity as a hard rule: every doc has exactly one designated purpose, and content that doesn't serve it (dated changelog-style entries, cycle/task history outside `ROADMAP.md`'s own rows, decision-reversal prose, verification evidence, restated `AGENTS.md` rationale, unresolved investigations dressed as settled traps) is banned, checked live on every generate/update, not just at cleanup time. New `/pipeme clean`: sweeps existing docs for purity violations (deleted on confirmation, no migration), misfiled open investigations (moved to `ROADMAP.md`'s Risks table), and cross-file duplicate facts (deduped to the cheaper-to-load copy).
