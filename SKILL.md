...
7. **Document purity is non-negotiable.** Every doc has exactly one designated purpose — the Contents column in the Output Files table, and the template in `output-templates.md` — and nothing else earns a place in it. An entry is a fact: 1–3 lines, no more. Banned everywhere, no exceptions: dated changelog-style status lines (`✅ Fixed {date}`, "was: X"), cycle/task/phase narrative outside `ROADMAP.md`'s own rows, decision-history or reversal prose ("reverses the earlier rule...", "used to be X"), verification/evidence detail (test counts, load-test results — that's a testing log's job), and rationale that's already homed in `AGENTS.md`'s `## Why …` sections — point to it, never restate it. If content doesn't fit the doc's designated purpose, cut it; don't soften it into "context." When in doubt, cut — git already has the history. `/pipeme clean` sweeps existing docs for drift against this rule; every generation and update step applies it live, so drift shouldn't accumulate in the first place.
...
| `/pipeme clean` | Clean Mode (below): sweep every existing doc for content that violates Golden Rule 7 (Document purity) and strip it. |
...
6. **Apply, and route the rationale.** Write the changes. **PipeMe keeps no changelog** — git history is the record of *what* changed, so don't duplicate it in prose. What git can't hold is *why a rule exists*, and that has one home: if the change creates or modifies a constraint whose reasoning an agent would otherwise "helpfully" undo — an over-broad matcher, a deliberate exception to a convention, a file that must stay frozen, a doc deliberately absent — add or update a `## Why …` section in `AGENTS.md`. Nothing else gets logged. A change that alters no rule's reasoning leaves no doc trace beyond the files it touched. Whatever lands in the target doc must still satisfy Golden Rule 7 — the fact in 1–3 lines, not the narrative that led to it.
7. **Maintenance check (every run, cheap).** If the root `CLAUDE.md` has drifted past ~1,000 tokens, say so and propose what should move to a nested file — don't silently let the always-on cost creep. Same pass, two more flags: any touched `ROADMAP.md` row that violates the completed-row format (see output-templates.md — build narrative, evidence, rationale, or warnings inside a task row), and any `AGENTS.md` `## Why …` entry grown past its cap (bold one-line rule + ≤3 lines rationale + pointer). Flag, propose the routing fix, don't silently rewrite.
8. **Drift check on touched docs (cheap, scoped).** Before writing, verify the docs being changed still agree with each other on the affected area (e.g. a ROADMAP task referencing a component TECH_SPEC no longer describes). Report contradictions as flags for the user to resolve — never pick a side silently. (Full-suite audit happens at cycle close in `/pipeme next`; there is no standalone review command as of v1.7.)
9. Confirm completion with a short summary of what changed and where.

### Clean Mode (`/pipeme clean`)

A purity sweep, not a drift audit — cross-doc consistency is Update Mode's touched-doc check and Next Cycle's full-suite audit. Clean Mode asks one question per doc: does every line still serve that file's one designated purpose (Golden Rule 7)? Nothing here compares docs against each other or against the codebase.
...
2. **Scan each for purity violations** against Golden Rule 7's banned list: dated changelog-style entries, cycle/task/phase narrative outside `ROADMAP.md`'s own rows, decision-history or reversal prose, verification/evidence detail, rationale that duplicates (or belongs in) `AGENTS.md`'s `## Why …` sections, and any entry longer than the 1–3 line standard for what it's stating.
3. **Present findings as a flagged list, grouped by file** — quote or summarize the offending text, name which part of Golden Rule 7 it breaks, and show what the trimmed line would read as. No file is touched at this step.
4. **Confirm before applying.** The user can approve all, approve per-file, or reject individual flags. **Clean Mode deletes, full stop — it never relocates.** No salvage pass, no "move it to AGENTS.md first." Git history is the only backup; that's the deliberate tradeoff for a sweep that's simple and predictable instead of one that quietly grows `AGENTS.md` on every run.
5. **Apply approved cuts.** Rewrite only the flagged spans — leave everything else in the file untouched, including formatting and surrounding structure.
6. **Report the result**: per file, what was removed (one line each) and the new measured size (token-budgeting command in `output-templates.md`).
...
**Archive sweep (mandatory):** before archiving `ROADMAP.md`, sweep accreted content out of task rows: build narratives, verification evidence, file inventories, rationale, and warnings that violate the completed-row format. Route each per the routing rule — *why* → `AGENTS.md` `## Why …` (capped format), evidence → the project's testing/verification log if one exists, everything else → nowhere (git already has it). The archived file should read like Phases 1–5 of a fresh roadmap: task, owner, status, criteria. Also flag any `AGENTS.md` entry past its cap while you're in there.
...
| `AGENTS.md` | **Accumulate** | Always grows, never archives. Retro lessons and new constraint reasoning get added as `## Why …` sections. Update the cross-tool map if new surfaces appeared. Never remove a `## Why …` section unless the constraint it explains was removed. |
...
**Purity check (mandatory, every doc, live).** Before finalizing any document, every line must satisfy Golden Rule 7 — it serves that file's one designated purpose, stated as a fact in 1–3 lines. No narrative, no history, no restated rationale, no dated status entries. Write it clean the first time; `/pipeme clean` exists for drift that accumulates later (imports, manual edits, older versions), not as a substitute for generating correctly now.
