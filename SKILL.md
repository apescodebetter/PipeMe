...
A purity sweep, not a drift audit — cross-doc consistency beyond duplication is Update Mode's touched-doc check and Next Cycle's full-suite audit. Clean Mode checks three things per doc: does every line serve that file's designated purpose (Golden Rule 7), is anything here an unresolved investigation misfiled as a settled trap, and is anything here a copy of a fact stated elsewhere instead of a pointer to it (Golden Rule 6). Nothing here reads the codebase.
...
2. **Scan for three finding types:**
   - **Purity violations** (Golden Rule 7): dated changelog-style entries, cycle/task/phase narrative outside `ROADMAP.md`'s own rows, decision-history or reversal prose, verification/evidence detail, restated `AGENTS.md` rationale, any entry past the 1–3 line standard.
   - **Misfiled investigations**: an entry describing an active, unresolved bug or open question rather than a settled constraint's reasoning — it isn't a "why," it's a work item wearing a trap's clothes.
   - **Cross-file duplicates**: the same fact stated in two docs instead of once with a pointer — most often an always-loaded `CLAUDE.md` constraint restated in `AGENTS.md`, or a decision repeated in both `PRD.md` and `TECH_SPEC.md`.
...
6. **Report the result**: per file, what was cut, moved, or deduped (one line each), and the new measured size (token-budgeting command in `output-templates.md`).
