...
`update`, `review`, `phase`, `retro`, `handoff`, `claude.md`, and `next` all require an existing project (docs already generated by a prior `new`/`quick`/`full`/`this` run). If no docs are found, say so plainly and suggest starting one instead of improvising.
...
**Preconditions:**
1. **Completion check (mandatory).** Read `ROADMAP.md` and count undone tasks. If more than a handful remain, say so: *"Phase {N} still has {X} open tasks. Did you want `/pipeme retro` first, or are you shelving those tasks to move on?"* Proceed only with explicit confirmation — don't silently abandon unfinished work.
2. **Read all existing docs** — `PRD.md`, `TECH_SPEC.md`, `ROADMAP.md`, `AGENTS.md`, `TEST_PLAN.md`, `DIAGRAMS.md`, `CLAUDE.md`, and any nested `{dir}/CLAUDE.md` files. These are the baseline; the interview only asks what they can't answer.
...
| `TEST_PLAN.md` | **Extend** | Test coverage is cumulative — existing critical paths and QA gates stay. Add new entries for the new cycle's features. |
...
- Not `/pipeme retro` — retro feeds lessons into the existing roadmap; next closes the current roadmap and opens a new one.
