...
`update`, `phase`, `handoff`, `claude.md`, and `next` all require an existing project (docs already generated by a prior `new`/`quick`/`full`/`this` run). If no docs are found, say so plainly and suggest starting one instead of improvising.
