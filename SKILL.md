...
7. **Document purity is non-negotiable.** Every doc has exactly one designated purpose — the Contents column in the Output Files table, and the template in `output-templates.md` — and nothing else earns a place in it. An entry is a fact: 1–3 lines, no more. Banned everywhere, no exceptions: dated changelog-style status lines (`✅ Fixed {date}`, "was: X"), cycle/task/phase narrative outside `ROADMAP.md`'s own rows, decision-history or reversal prose ("reverses the earlier rule...", "used to be X"), verification/evidence detail (test counts, load-test results — that's a testing log's job), an open or unresolved investigation, known-but-unfixed issue, or stale doc awaiting rework written up as if it were a settled trap (none of these are a "why" — they belong in `ROADMAP.md`'s Risks table, or a task, until actually resolved, then get promoted if they turn out to explain a constraint), and rationale that's already homed in `AGENTS.md`'s `## Why …` sections — point to it, never restate it. If content doesn't fit the doc's designated purpose, cut it; don't soften it into "context." When in doubt, cut — git already has the history.
   ...
   Two calibrations, not loopholes: a bare task/cycle-ID used as a pointer (`see 12.2`, `follows 7.12`) is fine — the banned thing is narrating what happened in that cycle (`Cycle 2 retro: ...`, `a Cycle 3 default judgment call`), not citing one. And a past incident cited as the load-bearing reason for a non-obvious current design **is** legitimate rationale — the `## Why …` template exists for exactly this — while the violation is dated/evidentiary framing (`Fixed {date}`, request counts, "shipped as a real bug") wrapped around it. When trimming, cut the framing, keep the reasoning.
