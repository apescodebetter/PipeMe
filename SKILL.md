...
name: pipeme
version: 1.11
description: PipeMe — a virtual software company that turns an app/SaaS idea into AI-agent-ready development documentation through the /pipeme command interface. Produces a context-budgeted documentation suite from scratch or from the current project (PRD, technical spec, task-decomposed roadmap, layered CLAUDE.md bootstrap, AGENTS.md cross-reference map, Mermaid diagrams). Does not produce changelogs or brand/design-system docs. Triggers only on /pipeme commands or explicit mentions of PipeMe.
---
...
5. **Small verifiable tasks, verification named up front.** Roadmap tasks are table rows — `| ID | Task | Owner | Status | Depends on | Done when |` — never bullet lists or paragraph blocks. The `Done when` cell is 1–2 phrases naming the criterion and its verification method (`[test]`, `[live]`, or `[human]`), nothing more — no implementation detail, no constraint history, no lineage ("carried from Cycle N"). "Done" claimed without the named verification isn't done.
...
7. **Document purity is non-negotiable.** Every doc has exactly one designated purpose — the Contents column in the Output Files table, and the shape in `document-rules.md` — and nothing else earns a place in it. An entry is a fact: 1–2 phrases, no more — not a sentence of setup plus a sentence of detail, 1–2 short phrases, total. Banned everywhere, no exceptions: dated changelog-style status lines (`✅ Fixed {date}`, `As of {date}, X is connected`, "was: X"), cycle/task/phase narrative or markers outside `ROADMAP.md`'s own rows — including a bare decorative prefix on an otherwise-fine rule (`⭐ Cycle 3: ...`, `New in Cycle N`) — a rule stands on its own; when it was added is git's job, not the rule's, decision-history or reversal prose ("reverses the earlier rule...", "used to be X"), bug-discovery-and-fix narrative in any form, dated or not ("X was missing, it broke, we fixed it with Y" — restate as the rule and what it guards against, never the incident), verification/evidence detail (test counts, load-test results — that's a testing log's job), an open or unresolved investigation, known-but-unfixed issue, or stale doc awaiting rework written up as if it were a settled trap (none of these are a "why" — they belong in `ROADMAP.md`'s Risks table, or a task, until actually resolved), and rationale of any kind, anywhere. If content doesn't fit the doc's designated purpose, cut it; don't soften it into "context." When in doubt, cut — git already has the history.
   ...
   `AGENTS.md` holds no rationale at all — not a compressed entry, not a consequence-framed one-liner, nothing past a routing table and the fixed Capability limits section. A constraint's *why*, when it's non-obvious enough to need one, lives in a code comment at the point of the constraint — where whoever's about to change that code will actually see it — never in a separate doc. This is the default; the agent doesn't add a rationale section back on its own judgment, ever, even for one line. The user can ask for one in a specific case — that's their call, not a standing exception.
...
| `/pipeme clean` | Clean Mode (below): sweep every existing doc for purity violations, misfiled investigations, and cross-file duplicates. |
...
6. **Generate docs with a current-state anchor.** Same output files, plus ROADMAP.md gains a `## Phase 0 — Current State` section using the same table shape as any other phase: `| ID | Task | Owner | Status | Depends on | Done when |` — existing capabilities as `done` rows, gaps/tech debt as `todo` rows with 1–2 phrase criteria. The forward roadmap builds from Phase 0; 🤖/👤/🤝 ownership tagging + Human Action Queue apply as normal.
...
5. **Route the change to the right cost class.** A new rule or convention goes to exactly one file: safety-critical and universal → root `CLAUDE.md`; surface-specific → that directory's `CLAUDE.md`; reasoning behind an existing rule → a code comment at that rule, never a doc. Never add the same rule to two files, and never grow the root file with something only one surface needs. If a change makes a doc newly relevant, add a **routing-table row with its token cost** — not a bootstrap read step.
6. **Apply in the correct shape, and route the rationale.** Write the changes **in the structural shape defined by `document-rules.md`** — a ROADMAP task is a table row with a 1–2 phrase Done-when cell, never a bullet list or paragraph block; a deferred item is a table row, not a paragraph bullet; a DIAGRAMS.md addition is a mermaid fence with `title:` frontmatter, never prose outside it; an AGENTS.md addition is a routing-table row, nothing else. **PipeMe keeps no changelog** — git history is the record of *what* changed, so don't duplicate it in prose. What git can't hold is *why a rule exists*, and that has one home: if the change creates or modifies a constraint whose reasoning an agent would otherwise "helpfully" undo — an over-broad matcher, a deliberate exception to a convention, a file that must stay frozen — add or update a **code comment at the point of the constraint**. Nothing goes in `AGENTS.md`; it holds no rationale, only a routing table. Whatever lands in a doc must still satisfy Golden Rule 7 — the fact in 1–2 phrases, not the narrative that led to it. If the change surfaces an unresolved bug or open question rather than a settled decision, it goes in `ROADMAP.md`'s Risks table.
7. **Maintenance check (every run, cheap).** Run the purity linter (`output-templates.md`) scoped to the touched docs — it's a `grep`/`awk` pass, cheap regardless of how small the change was, and it's what actually catches an `AGENTS.md` change that added anything beyond a routing row, or a stray `⭐`, without reading every line for quality. Then verify the touched doc's structure matches its shape in `document-rules.md` — the linter catches content violations but not structural ones (a bullet-list task with perfect 1–2 phrase content still fails the shape). If the root `CLAUDE.md` has drifted past ~1,000 tokens, say so and propose what should move to a nested file — don't silently let the always-on cost creep. Flag anything the linter or a read finds matching Clean Mode's four finding types (purity violations, structural shape violations, misfiled investigations, cross-file duplicates) — propose the fix, don't silently rewrite. This is a quick pass on what you just touched, not a full sweep; point to `/pipeme clean` for anything broader.
...
A purity and structural sweep — not a drift audit (cross-doc consistency beyond duplication is Update Mode's touched-doc check and Next Cycle's full-suite audit). Clean Mode checks four things per doc: does every line serve that file's designated purpose (Golden Rule 7), does the file match its shape from `document-rules.md` (structure, not just content), is anything here an unresolved investigation misfiled as a settled trap, and is anything here a copy of a fact stated elsewhere instead of a pointer to it (Golden Rule 6). Nothing here reads the codebase.
...
2. **Run the purity linter first** (`output-templates.md`, Mandatory Mechanical Checks) over the whole suite — it surfaces markers, dated status, incident-phrasing, any `## Why …` heading in `AGENTS.md` at all, and `DIAGRAMS.md`/`ROADMAP.md` shape violations mechanically, in seconds, with no risk of missing one three files into the read. Every hit is a candidate, not a verdict — a hit on the incident-phrasing blocklist might be a false positive; a `## Why …` heading in `AGENTS.md` never is. Then read for the finding types below, since the linter can't judge structural shape violations, misfiled investigations, cross-file duplicates, or subtler purity violations that dodge the keyword list.
3. **Scan for four finding types:**
   - **Purity violations** (Golden Rule 7): dated changelog-style entries, cycle/task/phase narrative outside `ROADMAP.md`'s own rows, decision-history or reversal prose, verification/evidence detail, any entry past the 1–2 phrase standard, and — in `AGENTS.md` specifically — anything at all beyond the routing table and the fixed Capability limits section. `AGENTS.md` has no budget to exceed; any rationale content there, of any length, is a purity violation, not a judgment call.
   - **Structural shape violations** (`document-rules.md`): content in the wrong format for its file. Each doc has one structural shape — not a guideline, the literal skeleton. Concrete violations: `ROADMAP.md` tasks as bullet lists or paragraph blocks instead of the `| ID | Task | Owner | Status | Depends on | Done when |` table, multi-sentence `Done when` cells instead of 1–2 phrases, deferred items as paragraph bullets instead of a `| Item | Status | Blocker |` table, paragraph blocks floating between or around tables, `DIAGRAMS.md` content outside a mermaid fence (except the file title), `AGENTS.md` content outside the routing table and Capability limits section, `PRD.md` features or metrics as paragraphs instead of their defined shapes. The fix is always a restructure into the correct shape — never "it's fine because the content is good."
   ...
   - **Cross-file duplicates**: the same fact stated in two docs instead of once with a pointer. Check especially hard around `CLAUDE.md`, since duplication there is the most expensive kind — everywhere else it's paid once (on-demand) at worst; here it can be paid twice, every session, forever: a root `CLAUDE.md` constraint restated (not just pointed to) in a nested `{dir}/CLAUDE.md`, or the same surface-spanning rule written out in full in two different nested files instead of one-with-a-pointer. Also check `PRD.md`/`TECH_SPEC.md` for a repeated decision.
4. **Present findings as a flagged list, grouped by file**, each tagged with its type and the action it implies:
   - Purity violation → **cut** (quote the offending text, show the trimmed or removed result). In `AGENTS.md`, this is never a rewrite-to-fit-the-shape — there is no rationale shape in this file anymore, so the whole entry goes.
   - Structural shape violation → **restructure** (show the content in its correct shape — e.g. a bullet-list task rewritten as a table row with 1–2 phrase Done-when, a paragraph deferred item as a table row).
   - Misfiled investigation → **move to `ROADMAP.md`'s Risks table** (show the Risk / Impact / Mitigation row it becomes).
   - Cross-file duplicate → **keep the copy in the file whose designated purpose it actually matches, cut the other to a pointer or remove it entirely** — routed the same way Update Mode step 5 routes new content (safety-critical/universal → `CLAUDE.md`; surface-specific → that directory's `CLAUDE.md`; scope/decisions → `PRD.md`; architecture → `TECH_SPEC.md`). Load cost is a side effect of that placement, never the reason for it.
   No file is touched at this step.
5. **Confirm before applying**, per flag or in bulk. **Purity violations are cut in place, full stop — no relocating them to another file, no "move it to AGENTS.md first."** Cutting an entry that's pure incident narrative with a real rule buried in it means rewriting it into the rule and its consequence and moving that into a code comment at the point of the constraint (see the worked example in `output-templates.md`) — not into `AGENTS.md`, which holds none of it. **Structural shape violations are restructured, not left as-is** — "the content is accurate" is never a reason to keep the wrong format. Misfiled investigations and cross-file duplicates are the only two finding types that leave the doc entirely, and only because both name a concrete, already-existing destination.
6. **Apply approved actions** — cuts, restructures, Risks-table moves, and dedup — touching only the flagged spans and their named destinations. Leave everything else untouched, including formatting and surrounding structure.
7. **Report the result**: per file, what was cut, restructured, moved, or deduped (one line each), and the new measured size (token-budgeting command in `output-templates.md`).
...
2. Break that phase's existing tasks into smaller, independently verifiable sub-tasks — same table shape (`| ID | Task | Owner | Status | Depends on | Done when |`), same 1–2 phrase `Done when` cells, same rules as Step 3's feasibility pass. Sub-tasks of an already-started task inherit `in-progress` reality honestly — don't reset finished work to `todo`.
...
2. Produce one file, `HANDOFF.md`: project summary (from PRD), current phase and what's done vs. pending (from ROADMAP), key architecture decisions (from TECH_SPEC, decisions only, not full spec), hard constraints (from the root `CLAUDE.md`). For recent history, read `git log --oneline -20` rather than any changelog; for non-obvious constraint reasoning, point to the code comment at that constraint rather than re-explaining it. Keep it to roughly one screen — this is a brief, not a re-export of every doc.
...
5. **Source facts from the docs, not the user**: read `ROADMAP.md` for the current phase (first phase with an undone task), `PRD.md` for the product name, and the actual repo tree for surfaces and commands. Do not ask anything the docs answer.
...
2. **Built-in mini-retro (Round 0, mandatory, 3–5 questions).** Before planning forward, ask: what worked well in the completed cycle, what was harder than expected, what the user would do differently, and whether any shipped features underperformed or surprised. Feed answers into the Risks table where they flag a recurring problem; a lesson that explains why a specific constraint must stay as-is goes into a code comment at that constraint, never into `AGENTS.md`.
...
**Archive sweep (mandatory):** before archiving `ROADMAP.md`, apply Clean Mode's full scan — all four finding types (purity violations, structural shape violations, misfiled investigations, cross-file duplicates). Structural shape is as important as content: every task must be a table row (`| ID | Task | Owner | Status | Depends on | Done when |`), every deferred item a table row (`| Item | Status | Blocker |`), every Done-when cell 1–2 phrases — no bullet lists, no paragraph blocks, no narrative. Build narratives, verification evidence, rationale, and warnings route the same way: *why* → a code comment at the constraint, evidence → the testing log, everything else → nowhere (git has it). The archived file should read like Phases 1–5 of a fresh roadmap: tables only. While you're in `AGENTS.md`, run Clean Mode's full scan on it — confirm it's still just the routing table and Capability limits, nothing else.
...
| `AGENTS.md` | **Regenerate (routing table only)** | Same treatment as `CLAUDE.md`: rebuild the table from the current doc/surface list, nothing else. `AGENTS.md` holds no rationale to prune or accumulate — a rule's reasoning lives in a code comment at that rule, not here. |
...
Read `references/document-rules.md` for the exact shape of each file, and `references/output-templates.md` for cycle-close mechanics and the mandatory checks. Create each document as a separate markdown file. Follow the shapes strictly — they are optimized for AI-agent consumption.

**Purity and shape check (mandatory, every doc, live).** Before finalizing any document, every line must satisfy Golden Rule 7 — it serves that file's one designated purpose, stated as a fact in 1–2 phrases — **and** sit in the structural shape `document-rules.md` defines for that file (tables where the shape says tables, mermaid fences where it says fences, nothing free-floating between them). No narrative, no history, no restated rationale, no dated status entries. **Run the purity linter** (`output-templates.md`, Mandatory Mechanical Checks) over the generated files and resolve every flag before calling the pass done — this isn't optional review, a prose rule alone has already been shown not to hold under real generation. The linter catches content violations but not structural ones, so check the shape by eye against `document-rules.md` too. Write it clean the first time; `/pipeme clean` exists for drift that accumulates later (imports, manual edits, older versions), not as a substitute for generating correctly now.
...
**`CLAUDE.md` generation (both modes):** always generate the root `CLAUDE.md` **plus one nested `{dir}/CLAUDE.md` per major code surface** the architecture implies (e.g. `frontend/`, `backend/`, `extension/`, `infra/`). See `document-rules.md` for both shapes and `output-templates.md` for the token budget.
...
| `AGENTS.md` | Pure cross-reference map — what's where and when to read it, no rationale content | ✅ | ✅ |
